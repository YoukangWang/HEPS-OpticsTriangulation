function [delay] = CalculatePacketDelay(vid, fps, bandwidthGbps)
...
% 增加可选参数 bandwidthGbps，默认值为 1 (千兆网)
if nargin < 3
    bandwidthGbps = 1; 
end
...
% 使用动态传入的带宽计算利用率
max_bandwidth_bps = bandwidthGbps * 10^9; 
ratio_GigeUtilized = bits_perSecond / max_bandwidth_bps;

% 安全保护：如果计算出的数据量已经超过了物理网卡带宽上限
if ratio_GigeUtilized >= 1
    warning('Calculated data rate (%.2f Gbps) exceeds or equals the network link capacity (%.1f Gbps). Packet delay cannot fix this; please lower FPS or ROI.', bits_perSecond/10^9, bandwidthGbps);
    ratio_GigeNotUtilized = 0;
else
...
fprintf(1, '\nUsed %.1f-GigE bandwidth: %.1f %%\n', bandwidthGbps, 100 * ratio_GigeUtilized);
